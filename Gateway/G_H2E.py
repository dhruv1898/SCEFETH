...
def store_test_data_H2E(data):
    for i in range(0, len(test_result_h2e)):
        try:
            ...
            df.columns=['Date-Time','Habitat','Enddevice','Payload', 'Packet Number', 'Direction']
        except:
            column_names = ['Date-Time','Habitat','Enddevice','Payload', 'Packet Number', 'Direction']
            df = pd.DataFrame(columns = column_names)
            continue
    
    data = data.split('/')
    df2 = pd.DataFrame({"Date-Time":[data[0]],"Habitat":[data[1]],"Enddevice":[data[2]],"Payload":[data[3]], 'Packet Number':[data[4]], 'Direction':[data[5]]})
...
def toenddevice(message, lora):
    ...
    save = datetimestamp_h2e + '/' + enddev[0] + '/' + message_1[0] + '/' + enddev[1] + '/' + enddev[2] + '/'+ 'G2E'
...
def handle_Habitat(conn, addr, lora):
    ...
    while connected:
        ...
        if msg_length:
            ...
            # Store H2E Multihop test data
            datetimestamp_h2e = str(time.time()+correction)
            save = datetimestamp_h2e + '/' + enddev[0] + '/' + message_1[0] + '/' + enddev[1] + '/' + enddev[2] + '/'+ 'H2G'
            store_test_data_H2E(save)
