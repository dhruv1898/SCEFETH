...
def LoRa_send(data, s):
    global offset
    s.send(data)
    sent_time = mpy_decimal.DecimalNumber(time.time()) + offset
    return sent_time
...
def LoRa_recv(s):
    ...
    received_time = mpy_decimal.DecimalNumber(time.time()) + offset   
...
def time_sync(s):
    ...
    sync_req = 'N:ID002:'+str(time.time())
    print(sync_req)
...
def data_send(file, s):
    global c,i
    #while True:
    pycom.rgbled(0xff00)  
    if i == 30:
        pycom.rgbled(0x7f7f00)
        data_send = 'D:ID002: Moin! Temperature value =  ' + 'Payload of Data packet for ID002' + ':' + str(c) # packet format
