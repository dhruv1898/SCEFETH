...
class DecimalNumber:
    ...
    def __init__(self, number=0, decimals = 0) -> None:
    ...
    def pi(cls) -> "DecimalNumber":
        ...
        if DecimalNumber.PI_SCALE >= DecimalNumber.get_scale():
            s = DecimalNumber(DecimalNumber.PI_NUMBER, DecimalNumber.PI_SCALE)
        else:
            # Calculates PI
    ...
    def e(cls) -> "DecimalNumber":
        ...
        if DecimalNumber.E_SCALE >= DecimalNumber.get_scale():
            e = DecimalNumber(DecimalNumber.E_NUMBER, DecimalNumber.E_SCALE)
        else:
    ...
    def ln2(cls) -> "DecimalNumber":
        ...
        if DecimalNumber.LN2_SCALE >= DecimalNumber.get_scale():
            e =  DecimalNumber(DecimalNumber.LN2_NUMBER, DecimalNumber.LN2_SCALE)
        else:
            scale = DecimalNumber.get_scale()
    ...
    def exp(self, inc_scale = True) -> "DecimalNumber":
        """Calculates exp(n)
    ...
    @staticmethod
    def _exp_lt_1(n: "DecimalNumber", inc_scale = True) -> "DecimalNumber":
    ...
    def _make_integer_comparable(n1: "DecimalNumber", n2: "DecimalNumber") -> Tuple[int]:
        ...
        """
        max_decimals = max(n1._num_decimals, n2._num_decimals)
        n1_number = n1._number
        if not n1._is_positive:
            n1_number = -n1_number
        n2_number = n2._number
    ...
    def __add__(self, other: "DecimalNumber") -> "DecimalNumber":
        ...
        max_decimals = max(self._num_decimals, other._num_decimals)

        a_factor = 10 ** self._num_decimals
        b_factor = 10 ** other._num_decimals

        a_integer = self._number // a_factor
        a_decimals = self._number % a_factor
        b_integer = other._number // b_factor
        b_decimals = other._number % b_factor
        ...
        c_factor = max(a_factor, b_factor)
        a_all = a_integer * c_factor + a_decimals
        b_all = b_integer * c_factor + b_decimals

        c_all = (a_all if self._is_positive else -a_all) + (b_all if other._is_positive else -b_all)
    ...
    def __mul__(self, other: "DecimalNumber") -> "DecimalNumber":
        if isinstance(other, int):
            other = DecimalNumber(other)
        a_integer = self._number if self._is_positive else -self._number
        b_integer = other._number if other._is_positive else -other._number
    ...
    def __truediv__(self, other: "DecimalNumber") -> "DecimalNumber":
        ...
        if b_integer != 0:
            c_factor = 10 ** (DecimalNumber.get_scale() + 2)
            c_integer = (a_integer * c_factor) // b_integer
            new_number = DecimalNumber(
    ...
    def __str__(self, thousands = False) -> str:
        ...
        #   12345 / 7: 0.0012345
        #   12345 / 8: 0.00012345
        str_number = str(self._number) if self._number >= 0 else str(-self._number)
        ...
        if thousands:
            pos_decimal = str_number.find(".")
            if pos_decimal == -1:
                first_part = str_number
                second_part = ""
            else:
                first_part = str_number[:pos_decimal]
                second_part = str_number[pos_decimal + 1:]
    ...
    def to_string_max_length(self, max_length, thousands = False) -> None:
        if max_length < 8:
            max_length = 8

        str_number = self.__str__(thousands)
        #   1,234,567,890.1234567
        #   If the number of characters before '.' is greater than max_length --> Overflow
        pos_point = str_number.find('.')
    ...
    def _reduce_to_scale(self) -> None:
        if self._num_decimals > DecimalNumber.get_scale():
            ...
            n = self._number
            s = self._num_decimals - DecimalNumber.get_scale()  # s: 6 - 3 = 3
            ds = (10 ** s)

            v = n % (ds * 10)  # v: n % 10**4 =  6789      1000
            b = v % ds         # b: v % 10**3 =   789
            a = v // ds        # a: v // 10**3 = 6
            m = ds // 2        # m: 10**3 // 2 = 500 (to be compared to b)
